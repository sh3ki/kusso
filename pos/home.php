...
<style>
    ...
    .cat-item button:focus,
    .cat-item button:hover {
        background: linear-gradient(135deg, #a95e2d 0%, #8b4513 100%) !important;
        transform: translateY(-4px) scale(1.02);
        box-shadow: 0 12px 20px rgba(0, 0, 0, 0.2), 0 6px 8px rgba(0, 0, 0, 0.12) !important;
    }
                    ...
                    <div style="display: flex;">
                        <div id="category-container">
                            ...
                            <div class="row justify-content-start align-items-center" id="cat-list">
                                <div class="col-auto cat-item large-mode mb-2" data-id="all">
                                ...
                                try {
                                    ...
                                    if ($stmt->rowCount() > 0) {
                                        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                                            $catName = strtolower(trim($row['name']));
                                            if ($catName === 'add ons' || $catName === 'addons') {
                                                $addons = $row;
                                            } else {
                                                $categories[] = $row;
                                            }
                                        }
                                        // Output all categories in alphabetical order except Add Ons
                                        foreach ($categories as $cat) {
                                            echo '<div class="col-auto cat-item large-mode mb-2" data-id="' . htmlspecialchars($cat['id']) . '"><button class="btn btn-primary">' . htmlspecialchars(ucwords($cat['name'])) . '</button></div>';
                                        }
                                        // Output Add Ons last if it exists
                                        if ($addons) {
                                            echo '<div class="col-auto cat-item large-mode mb-2" data-id="' . htmlspecialchars($addons['id']) . '"><button class="btn btn-primary">' . htmlspecialchars(ucwords($addons['name'])) . '</button></div>';
                                        }
                                    } else {
                        ...
                        <div id="prod-list-container" style="display: none; flex: 1;">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const categoryButtons = document.querySelectorAll('.cat-item button');
    ...
    function bindProductClicks() {
        document.querySelectorAll('.prod-item').forEach(product => {
            product.addEventListener('click', function(e) {
                ...
                fetch(`check_product_stock.php?product_id=${productId}`)
                    .then(response => response.json())
                    .then(data => {
                        ...
                        if (data.outOfStock && data.outOfStock.length > 0) {
                            ...
                            data.outOfStock.forEach(item => {
                                html += `<li><b>${item.name}</b>: OUT OF STOCK (${item.stock} ${item.unit})</li>`;
                            });
                            html += '</ul>';
                            document.getElementById('lowStockModalBody').innerHTML =
                        ...
                        if (data.insufficient && data.insufficient.length > 0) {
                            ...
                            document.getElementById('continueLowStock').onclick = function() {
                                ...
                                if (catType === 'Drinks') {
                                    ...
                                    addFoodToOrder(productId, productName, productOptions, parseFloat(productPriceRaw));
                                }
                            };
                ...
                function showDrinkSizeModal() {
                    ...
                    selectedDrinkProduct = { id: productId, name: productName, options: productOptions, prices };
                            ...
                            b.onclick = function() {
                                ...
                                fetch(`check_product_stock.php?product_id=${productId}&size=${btn.size}`)
                                    ...
                                    .then(data => {
                                        ...
                                        addDrinkToOrder(selectedDrinkProduct, btn.size, priceFloat, note, sugarLevel);
                                        // Reset form fields
    ...
    function addDrinkToOrder(product, size, price, note, sugarLevel) {
    ...
    function addFoodToOrder(productId, productName, productOptions, productPrice) {
    ...
    function updateOrderList() {
        ...
        orderItems.forEach(item => {
            ...
            let noteHtml = item.note ? `<div style='font-size:0.95em;color:#7a5c2e;'>Note: ${item.note}</div>` : '';
            row.innerHTML = `
