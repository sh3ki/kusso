...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if (isset($_POST['add_category_ingredient_btn'])) {
            ...
            if (!$category_id || !$ingredient_id || !$quantity_requirement || !$unit) {
            ...
            if ($check_stmt->fetch()) {
                throw new Exception("This ingredient is already linked to this category for size " . $size);
            }

            $stmt = $pdo->prepare("
                INSERT INTO category_ingredients (category_id, ingredient_id, quantity_requirement, unit, size)
                VALUES (:category_id, :ingredient_id, :quantity_requirement, :unit, :size)
            ");
            $stmt->execute([
                ':category_id' => $category_id,
                ':ingredient_id' => $ingredient_id,
            ...
            $_SESSION['success_message'] = "Ingredient linked to category successfully!";
        }
