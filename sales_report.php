...
<body class="sb-nav-fixed">
                                ...
                                <table class="products-table">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td class="avg-price">₱<?php echo number_format($avg_price, 2); ?></td>
                                            <td>
    ...
    <script>
        ...
        new Chart(document.getElementById('productTypeChart').getContext('2d'), {
            type: 'bar',
            data: {
                labels: topProductsData.slice(0,6).map(item => item.product_name.length > 15 ? item.product_name.substring(0,15) + '...' : item.product_name),
        ...
        const categoryData = topProductsData.reduce((acc, product) => {
            const category = product.product_name.split(' ')[0];
            acc[category] = (acc[category] || 0) + parseFloat(product.total_revenue || 0);
            return acc;
        }, {});
        new Chart(document.getElementById('productCategoryChart').getContext('2d'), {
            type: 'doughnut',
            data: { labels: Object.keys(categoryData).slice(0,5), datasets: [{ data: Object.values(categoryData).slice(0,5), backgroundColor: [chartColors.red, chartColors.purple, chartColors.orange, chartColors.teal, chartColors.pink], borderColor: '#fff', borderWidth: 2 }] },
...
<body>
    <table border="0" cellspacing="0" cellpadding="0" width="100%">
        ...
        <tr><td class="section-title" colspan="6">BEST SELLING PRODUCTS</td></tr>
        <tr>
            ...
            <td class="table-header">Avg Price</td>
            <td class="table-header">% of Total</td>
        </tr>`;
        ...
        <tr>
            ...
            <td class="${cellClass} text-right">${formatCurrency(avgPrice)}</td>
            <td class="${cellClass} text-right">${percentage}%</td>
        </tr>`;
        });
        ...
        function changePeriod(period) {
            ...
            switch(period) {
                case 'daily': startDate = new Date(); break;
                case 'weekly': startDate = new Date(); startDate.setDate(startDate.getDate() - 6); break;
                case 'monthly': startDate = new Date(); startDate.setDate(startDate.getDate() - 29); break;
            }
            const f = d => d.toISOString().split('T')[0];
            window.location.href = `?start_date=${f(startDate)}&end_date=${f(endDate)}&period=${period}`;
        }
